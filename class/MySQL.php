...
// 		$this->free();
	...
	public function get_one($sql) {
	    $cache = FCache::getInstance()->get($sql);
	    if ($cache){
	        return $cache;
	    }
		$this->query($sql, 1);
		...
		$row = $this->fetch();
		$this->free();
		FCache::getInstance()->set($sql, $row[0]);
		return $row[0];
	}
	...
	public function get_row($sql, $fetch_mode = MYSQLI_ASSOC) {
	   $cache = FCache::getInstance()->get($sql);
		...
		$row = $this->fetch();
		$this->free();
		FCache::getInstance()->set($sql, $row);
		return $row;
	}
	...
	public function get_all($sql, $limit = null, $fetch_mode = MYSQLI_ASSOC) {
		...
		$this->free();
		FCache::getInstance()->set($sql, $all_rows);
		return $all_rows;
	}
